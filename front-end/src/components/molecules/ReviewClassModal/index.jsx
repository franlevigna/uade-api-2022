...
export const ReviewClassModal = ({reviewModalData, handleClose, classData, refetch}) => {
    ...
    const userReview = classData.classes_reviews?.find((review) => review.userId === reviewModalData.user.id);
    ...
    const handleSubmit = async (values) =>{
        ...
        const payload = {
         ...
         try {
            ...
            Toast("¡Enhorabuena! La reseña se ha guardado!")
            handleClose()
            refetch()
        } catch (error) {
    ...
    const handleEdit = async (values) =>{
        ...
        const payload = {
         ...
         try {
            ...
            Toast("¡Enhorabuena! La reseña se ha modificado!")
            handleClose()
            refetch()
        } catch (error) {
            Toast(displayErrorMessage(error), "error")
  ...
  return <div>
      <Dialog open={reviewModalData.isOpen} onClose={handleClose} fullWidth>
        <DialogTitle>{`${!userReview? "Calificar" :"Modificar calificación para"} ${classData?.name}`}</DialogTitle>
        <DialogContent>
            <Box 
                id="reviewForm"
                component='form'
				...
				sx={{  width: {xs: "100%", sm: "70%"}, margin: "3rem auto 0"}}>
                    <Loading loading={isReviewClassLoading || isUpdateReviewLoading}/>
                        <Box  sx={{ flexGrow: 1, display: "flex", flexDirection:"column", alignItems: "center" , mb: 3}}>
                            <Rating
                                sx={{ flexGrow: 1,  }}
                                name='read-only'
                     ...
                     margin="dense"
                        ...
                        fullWidth
                        id='message'
                        label={isCommented? "Tu comentario": '¿Fue una buena elección?'}
                        ...
                        multiline
                        minRows={3}
                        InputProps={{readOnly: isCommented, disableUnderline: isCommented}}
                        disabled={isReviewClassLoading }
                    />
            </Box>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleClose} disabled={isReviewClassLoading}>Cancelar</Button>
          <Button disabled={isReviewClassLoading} type="submit" form="reviewForm" variant="contained" >{userReview? "Editar calificación" : "Calificar"}</Button>
        </DialogActions>
      </Dialog>
    </div>
}